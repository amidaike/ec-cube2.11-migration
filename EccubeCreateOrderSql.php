...
class EccubeCreateOrderSql
{
    ...
    /**
     * dtb_shippingのinsert文を作成
     *
     * @return bool
     */
    public function writeShippingSql()
    {
        $sql = 'select s.*, od.*, c1.name as name1, c2.name as name2  from dtb_shipping s'.
            ' left join dtb_shipment_item si ON s.shipping_id = si.shipping_id and s.order_id = si.order_id'.
            ' left join dtb_order_detail od on s.order_id = od.order_id'.
            ' left join dtb_products_class pc on od.product_class_id = pc.product_class_id'.
            ' left join dtb_class_combination cc ON pc.class_combination_id = cc.class_combination_id '.
            ' left join dtb_classcategory cd1 ON cd1.classcategory_id = cc.classcategory_id '.
            ...
            ' left JOIN dtb_class c1 ON cd1.class_id = c1.class_id '.
            ' left JOIN dtb_class c2 ON cd2.class_id = c2.class_id '.
            ' group by s.shipping_id, s.order_id';
        ...
        for ($count = 1; $count <= $turn; $count++) {
            ...
            for ($i = $loop; $i < $maxImport * $count; $i++) {
                ...
                $shipping_delivery_name = $item['shipping_num'] != '' ? "'".str_replace("'", "\'", $item['shipping_num'])."'" : 'NULL';
                ...
                $shipmentItemValues[] = "($shipping_id, $order_id, $product_id, $product_class_id, $id,"
            ...
            if (!empty($shipmentItemValues)) {
                $importShipmentItemSql = 'INSERT INTO dtb_shipment_item(item_id,order_id,product_id,product_class_id,'
    ...
    /**
     * dtb_order_detail、dtb_shipment_itemのinsert文を作成
     *
     * @return bool
     */
    public function writeOrderDetailSql()
    {
        ...
        $sql = 'SELECT distinct detail.*, pc.product_class_id, c1.name as name1, c2.name as name2 '.
            'FROM dtb_order_detail detail '.
            'INNER JOIN dtb_order o ON o.order_id = detail.order_id '.
            'left JOIN dtb_products_class pc ON detail.product_class_id = pc.product_class_id '.
            'left join dtb_class_combination cc ON pc.class_combination_id = cc.class_combination_id '.
            'left join dtb_classcategory cd1 ON cd1.classcategory_id = cc.classcategory_id '.
            'left join dtb_class_combination cc2 on cc2.class_combination_id = cc.parent_class_combination_id '.
            'left join dtb_classcategory cd2 ON cd2.classcategory_id = cc2.classcategory_id '.
