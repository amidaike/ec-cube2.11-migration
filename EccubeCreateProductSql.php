...
class EccubeCreateProductSql
{
    ...
    public function writeCategorySql()
    {
        ...
        // insert文
        $sql = 'INSERT INTO dtb_category (category_id, category_name, parent_category_id, level, rank, creator_id, create_date, update_date, del_flg, main_comment) VALUES'.PHP_EOL;
        ...
        foreach ($rows as $item) {
            ...
            $level = $item['level'];
            $rank = $item['rank'];
            $main_comment = $item['main_comment'] != '' ? "'".str_replace("'", "\'", $item['main_comment'])."'" : 'NULL';

            $values = "($id, $category_name, $parent_category_id, $level, $rank, $creator_id, $create_date, $update_date, $del_flg, $main_comment)";
            if ($item == $lastItem) {
    ...
    {
        $sql = 'SELECT pc.* FROM dtb_product_categories pc INNER JOIN dtb_products p ON pc.product_id = p.product_id INNER JOIN dtb_category c ON pc.category_id = c.category_id ORDER BY pc.product_id, pc.rank;';
    ...
    {
        $sql = 'SELECT distinct pc.*,p.deliv_date_id, cc.classcategory_id as classcategory_id1, cc2.classcategory_id as classcategory_id2 FROM dtb_products_class pc INNER JOIN dtb_products p ON pc.product_id = p.product_id'.
            ' left join dtb_class_combination cc ON pc.class_combination_id = cc.class_combination_id'.
            ' left join dtb_classcategory cd1 ON cd1.classcategory_id = cc.classcategory_id or cc.classcategory_id is null'.
            ' left join dtb_class_combination cc2 on cc2.class_combination_id = cc.parent_class_combination_id'.
            ' left join dtb_classcategory cd2 ON cd2.classcategory_id = cc2.classcategory_id'.
            ' where pc.product_class_id > 0 ORDER BY pc.product_class_id ASC';
        ...
        foreach ($rows as $item) {
            ...
            $class_category_id1 = $item['classcategory_id1'] == null ? 'NULL' : $item['classcategory_id1'];
            $class_category_id2 = $item['classcategory_id2'] == null ? 'NULL' : $item['classcategory_id2'];
    ...
    {
        $sql = 'SELECT * FROM dtb_product_status ORDER BY product_status_id ASC;';
